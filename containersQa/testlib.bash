...
function checkHardcodedJdks() {
  if [ "$OTOOL_jresdk" == "jre"  ] ; then
    ...
    cat $(getOldMvnVersionLog) | grep -e "Java version: 11.0.28" -e "Java version: 1.8.0_462" -e "Java version: 17.0.16" -e "Java version: 21.0.8"
  fi    
...
function setupAlgorithmTesting {
  ...
  local configFileName=$(bash $LIBCQA_SCRIPT_DIR/algorithmTesting/chooseAlgorithmConfigFile.sh $OTOOL_cryptosetup $OTOOL_BUILD_OS_VERSION)
